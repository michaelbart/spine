...
one or more fenced prompt blocks below (one for `product-spec.md`/
`feature-handoff.md`; three staged ones plus a closing note for
`ui-handoff.md`). Do not dump the raw file, comment markup included —
a reader in a terminal shouldn't have to see `<!-- -->` fencing:

1. **Restate the usage note in plain prose**, one short paragraph: when to
   use this prompt, where to paste it, and where the output should be
...
2. **Then print every fenced prompt block verbatim, each in its own code
   fence**, in the file's own order, with the file's own stage headings
   (`ui-handoff.md`'s three stages) as plain text labels above each
   one — so the human can copy exactly one block cleanly, without
   reformatting anything themselves.
