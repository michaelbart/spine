...
<!-- Optional — omit this whole section if this plan doesn't close out any
     ...
     future member tasks`. Only for a milestone member task (`--milestone
     <id>` given). /ship reads this to know which gap-<n> fenced entries to
     remove from milestone.md once this task's diff has actually shipped —
     never inferred from prose, only from this explicit citation ("no
     implicit resolution"). Citing a gap here that this plan's own `## The
     gist` doesn't actually address is caught the same way an unfounded
     `## Grounds on decisions` citation would be — at human plan review,
     not mechanically. -->
